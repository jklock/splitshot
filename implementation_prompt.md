...
You are working in the SplitShot repository. Implement the feature set below using the existing local-first architecture and the current browser code paths.
...
- Existing browser docs: [src/splitshot/browser/README.md](src/splitshot/browser/README.md), [src/splitshot/browser/static/README.md](src/splitshot/browser/static/README.md), [src/splitshot/ui/README.md](src/splitshot/ui/README.md), [src/splitshot/export/README.md](src/splitshot/export/README.md)
- Existing tests that will likely need updates: [tests/test_browser_static_ui.py](tests/test_browser_static_ui.py), [tests/test_browser_control.py](tests/test_browser_control.py), [tests/test_cli.py](tests/test_cli.py)
...
- `uv run splitshot` is the default browser launcher, and the shared CLI already exposes `--web`, `--no-open`, `--host`, `--port`, `--project`, `--check`, and `--log-level`.
...
- The browser shell is the production experience.
...
5. Clarify review and export labels and helper text.
   ...
   Keep the browser wording aligned across the product surface.
...
7. Enhance status and progress behavior for long-running actions.
   ...
   Make sure export progress uses the pipeline callback instead of a static status string.
...
8. Replace user-facing `upload` terminology with local-system wording.
   ...
   There are current matches in [src/splitshot/browser/static/app.js](src/splitshot/browser/static/app.js), [src/splitshot/browser/server.py](src/splitshot/browser/server.py), [src/splitshot/ui/README.md](src/splitshot/ui/README.md), [src/splitshot/browser/README.md](src/splitshot/browser/README.md), and [src/splitshot/browser/static/README.md](src/splitshot/browser/static/README.md). Update labels, hints, section names, status text, and docs so the UI no longer implies cloud uploading.
...
9. Update tests and documentation to match the new contract.
   ...
   Refresh [tests/test_browser_static_ui.py](tests/test_browser_static_ui.py) and [tests/test_browser_control.py](tests/test_browser_control.py) because they currently assert the old status strip, inspector, export log, and upload wording.
...
10. Harden the root `splitshot` command for production use.
   ...
   Keep `uv run splitshot` as the clean default launcher and make sure the existing command-line options continue to work exactly as they do now across the main entrypoint and the `splitshot-web` alias. Verify the help text and docs stay aligned with the real parser behavior.
...
11. Keep the browser shell as the only supported user-facing and tooling interface.

   Make the browser shell the only supported interface for validating controller, media analysis, merge, scoring, export, and filesystem behavior on each supported operating system. Keep the shared controller and derived state focused on the browser workflow and avoid reintroducing a second UI surface.

   Validation should rely on the browser workflow, runtime checks, and browser-focused automation or manual QA. Any platform-specific tooling checks should remain explicit and documented without bringing back a separate desktop window.

   Validate the browser surface on macOS, Windows, and Linux in a real GUI session, not headless.
...
13. Polish the GitHub-facing project presentation.
   ...
   Make the repository landing page feel complete and polished for first-time visitors. Update the main README and any supporting repo docs so the page clearly explains that SplitShot is a local-first browser application, how it runs on macOS, Windows, and Linux, and how the browser shell is the user experience.

   At minimum, include prominent status badges, a clearer feature overview, screenshots or an animated capture of the browser flow, a simple platform and browser support matrix, and a concise getting-started section that points to the new setup scripts and the normal `uv run splitshot` launch path. Keep the messaging local-first, browser-first, and avoid implying cloud services or headless deployment.
...
- Keep browser terminology aligned across the product surface.

...
- **Repaired clean-runner package validation.** The packaged release proof targets the current Trim source-card contract on every platform, while Test macOS validates a signed non-notarized DMG and reserves mandatory notarization for Build macOS and the publishing Release workflow.

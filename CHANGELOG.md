...
- **Repaired clean-runner package validation.** The packaged release proof targets the current Trim source-card and Queue output/log contracts on every platform, allows realistic shared-runner latency for profile persistence, and keeps mandatory notarization in Build macOS and the publishing Release workflow while Test macOS validates a signed non-notarized DMG.

...
def test_packaged_e2e_script_writes_export_artifact_under_artifacts_tree() -> None:
    ...
    assert "const exportDir = process.env.E2E_EXPORT_DIR || path.join(path.dirname(logDir), 'e2e-exports');" in script
    ...
    assert "return status.includes('Exported video to ');" in script
...
def test_ci_test_workflows_use_clip1_for_packaged_e2e_validation() -> None:
    for workflow in TEST_WORKFLOWS:
        ...
        assert "SPLITSHOT_E2E_VIDEO: docs/Clip1.MP4" in source, workflow.name
        if workflow.name == "test-windows.yml":
            ...
            assert "SPLITSHOT_E2E_SCOPE: export-proof" in source, workflow.name

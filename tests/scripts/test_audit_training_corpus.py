...
def _build_duplicate_stage_corpus(tmp_path: Path, synthetic_video_factory) -> Path:
    ...
    stage_b = synthetic_video_factory(
        name="stage-source-two", shot_times_ms=[800, 1100, 1450, 1750]
    )
...
def test_audit_training_corpus_emits_json_summary_for_directory(
...
def test_audit_training_corpus_writes_json_report_and_table_output(
    tmp_path: Path, synthetic_video_factory
) -> None:
    corpus_dir = _build_corpus_dir(tmp_path, synthetic_video_factory)
...
def test_audit_training_corpus_reports_duplicate_stage_groups(
    tmp_path: Path, synthetic_video_factory
) -> None:

...
def test_browser_foreground_server_reports_bind_failure(monkeypatch, capsys) -> None:
    ...
    server = BrowserControlServer(port=0)
    monkeypatch.setattr(
        server, "_build_httpd", failing_build_httpd.__get__(server, BrowserControlServer)
    )
...
def test_display_name_fallback_strips_browser_session_prefix() -> None:
    assert (
        display_name_for_path("/tmp/1234567890abcdef1234567890abcdef_Stage1.MP4", "None")
        == "Stage1.MP4"
    )
...
def test_browser_server_primes_export_runtime_on_construction(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        browser_server_module, "prepare_export_runtime", fake_prepare_export_runtime
    )
    server = BrowserControlServer(port=0)
    try:
...
def test_browser_control_api_imports_and_edits_video(synthetic_video_factory) -> None:
    ...
    try:
        ...
        assert state["project"]["analysis"]["shots"][0]["score"]["penalty_counts"] == {
            "procedural_errors": 1
        }
...
def test_browser_control_api_restores_original_split_and_score(synthetic_video_factory) -> None:
    ...
    try:
        ...
        moved_shot = next(
            shot for shot in state["project"]["analysis"]["shots"] if shot["id"] == shot_id
        )
...
def test_browser_export_route_syncs_scoring_overlay_and_merge_payloads_before_render(
    ...
    try:
        ...
        def fake_export_project(project, output_target, progress_callback=None, log_callback=None):
            ...
            assert project.merge_sources[0].placement.mode.value == "pip"
            assert project.merge_sources[0].placement.slot.value == "overlay"
            assert project.merge_sources[0].placement.target_kind.value == "primary_video"
            assert project.merge_sources[0].placement.target_source_id is None
        ...
        assert state["project"]["merge_sources"][0]["placement"]["mode"] == "pip"
        assert state["project"]["merge_sources"][0]["placement"]["slot"] == "overlay"
        assert state["project"]["merge_sources"][0]["placement"]["target_kind"] == "primary_video"
        assert state["project"]["merge_sources"][0]["placement"]["target_source_id"] is None
...
def test_browser_merge_source_analysis_state_exposes_sync_controls(
    ...
    try:
        ...
        first_source_id = state["project"]["merge_sources"][0]["id"]
        state = _post_json(f"{server.url}api/import/merge", {"path": str(second_merge_path)})
        second_source_id = state["project"]["merge_sources"][1]["id"]
        ...
        assert first_source["can_rerun_sync_analysis"] is False
        assert first_source["is_analyzed_sync_source"] is False
        assert second_source["supports_sync_analysis"] is True
        assert second_source["can_rerun_sync_analysis"] is True
        assert second_source["is_analyzed_sync_source"] is False

        reanalyzed = _post_json(
            f"{server.url}api/merge/source/analyze",
            {"source_id": second_source_id},
        )
        updated_sources_by_id = {
            item["id"]: item for item in reanalyzed["project"]["merge_sources"]
        }
        updated_first_source = updated_sources_by_id[first_source_id]
        updated_second_source = updated_sources_by_id[second_source_id]

        assert updated_first_source["supports_sync_analysis"] is False
        assert updated_first_source["can_rerun_sync_analysis"] is False
        assert updated_first_source["is_analyzed_sync_source"] is False
        assert updated_first_source["sync_analysis_status"] == "idle"
        assert updated_first_source["sync_analysis_message"] == ""
        assert updated_first_source["secondary_beep_time_ms"] is None
        assert updated_first_source["sync_offset_source"] == "manual"

        assert updated_second_source["supports_sync_analysis"] is True
        assert updated_second_source["can_rerun_sync_analysis"] is True
        assert updated_second_source["is_analyzed_sync_source"] is True
        assert updated_second_source["sync_analysis_status"] == "ready"
        assert updated_second_source["sync_offset_source"] == "auto"
        assert updated_second_source["secondary_beep_time_ms"] is not None
    finally:
        server.shutdown()
...
def test_browser_manual_merge_sync_after_auto_analysis_remains_authoritative_for_preview_and_export(
    ...
    try:
        ...
        first_merge_path = Path(
            synthetic_video_factory(name="merge-authoritative-secondary-a", beep_ms=650)
        )
        second_merge_path = Path(
            synthetic_video_factory(name="merge-authoritative-secondary-b", beep_ms=725)
        )
        ...
        auto_offset = next(
            ...
            for source in state["project"]["merge_sources"]
        ...
        assert first_source["can_rerun_sync_analysis"] is False
        assert first_source["is_analyzed_sync_source"] is False
        assert first_source["sync_analysis_status"] == "idle"
        assert first_source["secondary_beep_time_ms"] is None
        assert second_source["supports_sync_analysis"] is True
        assert second_source["can_rerun_sync_analysis"] is True
        assert second_source["is_analyzed_sync_source"] is True
        assert second_source["sync_offset_ms"] == auto_offset + 33
        assert second_source["sync_offset_source"] == "manual"
        assert second_source["secondary_beep_time_ms"] is not None
        ...
        def fake_export_project(project, output_target, progress_callback=None, log_callback=None):
            authoritative_source = next(
                source for source in project.merge_sources if source.id == second_source_id
            )
            assert authoritative_source.sync_offset_ms == auto_offset + 33
        ...
        exported_sources_by_id = {
            source["id"]: source for source in exported["project"]["merge_sources"]
        }
        assert exported_sources_by_id[first_source_id]["supports_sync_analysis"] is False
        assert exported_sources_by_id[second_source_id]["sync_offset_ms"] == auto_offset + 33
        assert exported_sources_by_id[second_source_id]["sync_offset_source"] == "manual"
...
def test_browser_control_loading_new_practiscore_csv_keeps_current_selection(
    tmp_path: Path,
) -> None:
    controller = ProjectController()
    server = BrowserControlServer(controller=controller, port=0)
    ...
    try:
        ...
        imported_box = next(
            box
            for box in state["project"]["overlay"]["text_boxes"]
...
def test_browser_project_open_restores_practiscore_state(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert (
            imported["project"]["scoring"]["imported_stage"]["competitor_name"]
            == "John Klockenkemper"
        )
...
def test_browser_control_api_covers_remaining_browser_routes(
    ...
    try:
        ...
        first_shot_id = state["project"]["analysis"]["shots"][0]["id"]
        added_shot_id = next(
            shot["id"] for shot in state["project"]["analysis"]["shots"] if shot["time_ms"] == 1750
        )
...
def test_browser_threshold_rerun_preserves_manual_shots_and_timing_events(monkeypatch) -> None:
    ...
    controller.add_timing_event(
        "reload", after_shot_id=first.id, before_shot_id=second.id, note="Keep me"
    )
...
def test_browser_file_picker_endpoint_imports_selected_primary_video(
    synthetic_video_factory,
) -> None:
    controller = ProjectController()
    server = BrowserControlServer(controller=controller, port=0)
...
def test_browser_file_picker_endpoint_preserves_secondary_display_name(
    synthetic_video_factory,
) -> None:
    controller = ProjectController()
    server = BrowserControlServer(controller=controller, port=0)
...
def test_choose_local_path_macos_falls_back_to_existing_parent_for_missing_media_path(
    ...
    assert "Choose added media" in script
...
def test_browser_project_open_replaces_stale_media_state(
    synthetic_video_factory, tmp_path: Path
) -> None:
    controller = ProjectController()
    server = BrowserControlServer(controller=controller, port=0)
    ...
    try:
        ...
        assert Path(reopened["project"]["primary_video"]["path"]).resolve() == video_path.resolve()
        assert controller.project_path == project_path
    finally:
        server.shutdown()


def test_browser_project_open_recovers_renamed_project_root_media(
    synthetic_video_factory, tmp_path: Path
) -> None:
    controller = ProjectController()
    server = BrowserControlServer(controller=controller, port=0)
...
def test_browser_project_open_restores_ui_state_and_export_output_path(
    ...
    try:
        ...
        assert reopened["project"]["ui_state"]["waveform_shot_amplitudes"] == {
            selected_shot_id: 1.5
        }
...
def test_browser_post_route_manifest_is_classified_and_disk_asserted() -> None:
    ...
    for route, test_names in DIRECT_PROJECT_JSON_ASSERTION_TESTS_BY_ROUTE.items():
        ...
        for test_name in test_names:
            ...
            assert route in test_source or route.lstrip("/") in test_source, (
                f"{test_name} does not exercise {route}"
            )
...
def test_browser_autosave_persists_analysis_scoring_timing_and_ui_changes_to_project_json(
    ...
    try:
        ...
        assert Path(saved["primary_video"]["path"]).resolve() == primary_path.resolve()
        assert (
            saved["analysis"]["beep_time_ms_primary"]
            == imported["project"]["analysis"]["beep_time_ms_primary"]
        )
        ...
        _post_json(
            ...
            {
                "settings": {"min_shot_interval_ms": 130, "shot_peak_min_spacing_ms": 230},
                "rerun": False,
            },
        )
        saved = _read_project_json(project_path)
        ...
        added_shot = next(
            shot
            for shot in added["project"]["analysis"]["shots"]
        ...
        saved = _read_project_json(project_path)
        assert any(
            proposal["id"] == restore_proposal["id"]
            for proposal in saved["analysis"]["timing_change_proposals"]
        )
        ...
        assert (
            next(
                proposal
                for proposal in saved["analysis"]["timing_change_proposals"]
                if proposal["id"] == restore_proposal["id"]
            )["status"]
            == "discarded"
        )
...
def test_browser_autosave_persists_overlay_merge_export_and_media_routes_to_project_json(
    ...
    try:
        ...
        second_source_id = uploaded_secondary_two["project"]["merge_sources"][1]["id"]
        saved = _read_project_json(project_path)
        ...
        _post_json(
            f"{server.url}api/merge/source",
            {
                ...
                "placement": {
                    ...
                    "target_source_id": second_source_id,
                },
            },
        )
        saved = _read_project_json(project_path)
        ...
        assert saved_source["placement"]["target_source_id"] == second_source_id
...
def test_browser_settings_section_reset_only_clears_selected_section(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(controller_module, "load_settings", lambda: controller_module.AppSettings())
    monkeypatch.setattr(controller_module, "save_settings", lambda settings: None)
...
def test_browser_project_open_recovers_practiscore_from_project_csv_folder(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert reopened["project"]["path"] == str(project_path)
        assert reopened["project"]["scoring"]["practiscore_source_path"] == str(
            staged_csv.resolve()
        )
        ...
        assert any(
            box["source"] == "imported_summary"
            for box in reopened["project"]["overlay"]["text_boxes"]
        )
        assert saved["scoring"]["match_type"] == "idpa"
...
def test_browser_project_save_bundles_imported_media_for_reopen(
    synthetic_video_factory, tmp_path: Path
) -> None:
    controller = ProjectController()
    server = BrowserControlServer(controller=controller, port=0)
...
def test_browser_media_endpoint_transcodes_pcm_audio_preview_once(
    monkeypatch, tmp_path: Path
) -> None:
    controller = ProjectController()
...
def test_browser_media_endpoint_returns_404_when_preview_disappears(tmp_path: Path) -> None:
    ...
    server = BrowserControlServer(controller=controller, port=0)
    server._prepare_browser_media = lambda path: (
        tmp_path / "missing-browser-preview.mp4",
        True,
        "test_missing_preview",
        "aac",
    )
    server.start_background(open_browser=False)
    try:
...
def test_browser_preview_timeline_ignores_container_duration_drift_when_other_video_metadata_matches() -> (
    ...
    assert (
        browser_server_module._browser_preview_matches_source_timeline(
            source_timeline, preview_timeline
        )
        is True
    )
...
def test_browser_preview_timeline_requires_exact_packet_match(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        browser_server_module, "_ffprobe_video_packet_csv", lambda path: packet_outputs[str(path)]
    )
...
def test_browser_media_cache_token_changes_when_same_primary_path_is_reimported(
    synthetic_video_factory,
) -> None:
    controller = ProjectController()
    server = BrowserControlServer(controller=controller, port=0)
...
def test_browser_control_api_updates_overlay_styles_and_scoring_preset(
    synthetic_video_factory,
) -> None:
    controller = ProjectController()
    server = BrowserControlServer(controller=controller, port=0)
    ...
    try:
        ...
        assert any(
            item["key"] == "PE" for item in state["scoring_summary"]["scoring_color_options"]
        )
        assert not any(
            "|" in item["key"] for item in state["scoring_summary"]["scoring_color_options"]
        )
        ...
        state = _post_json(
            f"{server.url}api/scoring/position", {"shot_id": shot_id, "x_norm": 0.2, "y_norm": 0.8}
        )
...
def test_browser_control_api_exports_mp4_and_exposes_ffmpeg_log(
    synthetic_video_factory, tmp_path: Path
) -> None:
    controller = ProjectController()
    server = BrowserControlServer(controller=controller, port=0)
...
def test_browser_control_api_export_uses_request_payload_overrides(
    synthetic_video_factory, tmp_path: Path
) -> None:
    controller = ProjectController()
    server = BrowserControlServer(controller=controller, port=0)

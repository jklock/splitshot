...
def test_browser_control_qa_matrix_documents_current_browser_suites() -> None:
    ...
    assert "| Project / import | project details, create/select project, project-folder display, gated PractiScore dashboard opener, gated manual PractiScore file import, metadata-only delete |" in matrix
    assert "| Media | active-stage primary intake or clear, added-media intake or removal, stage selection, stage media counts |" in matrix

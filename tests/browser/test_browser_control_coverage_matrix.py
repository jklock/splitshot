...
def test_browser_control_qa_matrix_documents_current_browser_suites() -> None:
    ...
    assert (
        "| Media | active stage selector, stage name, `Save`, `Delete`, `Add Stage`, persistent Primary/Added Media disclosures, `Add Primary`, primary asset `Replace`/`Clear`, `Set Primary`, `Remove`, `Add Media`, Media/Compose/video-stage file drop |"
        in matrix
    )
    assert (

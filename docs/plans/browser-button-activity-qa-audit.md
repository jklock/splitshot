...
| Edit | Left rail | Activates edit instructions for waveform editing. | `button.click`, `ui.tool.click`, `ui.tool.active`. |
...
| Place Score | Video stage | Places scoring coordinates for the selected shot on the video preview. | `button.click`, `api.start /api/scoring/position`, `api.success`. |
...
| Apply Threshold | Review pane | Re-runs detection threshold tuning against the current primary video. | `button.click`, `api.start /api/analysis/threshold`. |
...
| Apply Scoring | Scoring pane | Saves scoring enabled state, penalties, and ruleset. | `button.click`, `api.start /api/scoring/profile`, `api.start /api/scoring`. |
| Assign To Selected Shot | Scoring pane | Applies the chosen score letter to the selected shot. | `button.click`, `api.start /api/scoring/score`. |
| Apply | Overlay pane | Saves overlay position, badge size, badge colors, score colors, and opacity. | `button.click`, `api.start /api/overlay`. |
...
| Apply Merge | Merge pane | Saves merge layout, PiP size, and enabled state. | `button.click`, `api.start /api/merge`. |
...
| Apply | Layout pane | Saves quality, aspect ratio, and crop center settings. | `button.click`, `api.start /api/layout`. |

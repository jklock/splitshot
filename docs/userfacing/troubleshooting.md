...
| The wrong stage is active. | Select the correct stage in Media or Queue. The selected stage becomes the live stage for the rest of the app. |
...
| `Apply Active Stage Settings to Queued` copied something unexpected. | It copies stage-local settings and excludes markers. Re-select the affected stage and adjust as needed. |
...
| `Process Many` skips a stage. | Check whether the row is `Queued`, `Stale`, or `Failed`. |

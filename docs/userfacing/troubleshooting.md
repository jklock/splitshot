...
| The wrong stage is active. | Select the correct stage in Media or Queue. |
| A stage has no media. | Open Media and import the primary video for that stage. |
| Added media is attached to the wrong stage. | Select the correct row in Media, remove the wrong added media in Compose, then re-import it on the correct stage. |

## Compose, Trim, And Review

| Problem | Fix |
| --- | --- |
| Changes seem to affect the wrong stage. | Confirm the active stage in Media or Queue before editing. |

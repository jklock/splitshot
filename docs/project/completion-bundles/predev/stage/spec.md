...
- Added-media management must live in the Compose step, not in Project.
- When added media exists, the secondary waveform must appear beneath the primary waveform/info lane.
- Review added media must default to enabled when added media exists.
...
- focused regression coverage for Project import/home, Compose waveform/sync, Review defaults and summary authoring, marker styling, and top-bar status placement

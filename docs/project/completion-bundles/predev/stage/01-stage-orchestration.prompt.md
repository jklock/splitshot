---
description: "Archival alias for the canonical Stage shell reset orchestration prompt."
name: "01 Stage Shell Reset Orchestrator (Alias)"
argument-hint: "Optional Stage workflow, regression, or shell seam to prioritize"
agent: "agent"
---

This file is an archival alias for [orchestration.prompt.md](./orchestration.prompt.md).

Do not maintain this file independently. Read [master status](../../MASTER_STATUS.md) for current lane and program status, then use the unnumbered prompt as the canonical source.
...
- Preserve the editing flow order: `Project -> Compose -> Splits/Score -> Overlay -> Markers -> Review -> Export`.
...
- Treat Project-home defaults, primary-video import copy behavior, Compose waveform/sync, Review defaults/summary authoring, marker styling, and top-bar status placement as critical regressions if touched.

...
- PiP, splits, score, overlay, markers, review, export, timing, scoring, metrics, and ShotML Stage workflows
...
Stage is still the clearest editing surface in the product, but the current contract is stale.

Current facts that matter:

- Stage already contains the shell grammar the user wants reused everywhere.
- Project has accumulated automation/setup UI that interrupts the editing flow and includes placeholder-heavy sections.
- Match and Performance have been documented as separate shell families instead of Stage-shell variants.
- PiP waveform/default/sync, imported-summary authoring, review layout, marker-style inheritance, and top-bar status placement are still open integration issues.
- Existing completion ledgers for Stage, Match, and Performance describe the wrong target and must no longer be treated as signoff evidence for the redesign.
...
- the Stage editing workflow order: `Project -> PiP -> Splits/Score -> Overlay -> Markers -> Review -> Export`
...
- PiP: secondary media import, source roles, sync/alignment, waveform-secondary visibility
...
- PiP waveform visibility and playback sync

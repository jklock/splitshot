...
Performance already has meaningful backend and interaction proof, but it is still documented against the wrong UI target.
...
- The current bundle still treats Performance as a standalone shell and was left awaiting visual approval under that model.
- The user now wants Performance to reuse the Stage shell with data/graphs in the main area and detail/info below.
- Existing non-visual proof can be carried forward where the behavior remains unchanged, but the shell and visible workflow must be re-proven.

...
| Compose | add media, compose default settings collapse and restore, per-item card toggle/remove, per-item size/opacity/position/layout controls | `tests/browser/test_merge_export_contracts.py`; `tests/browser/test_timing_waveform_contracts.py`; `tests/browser/test_browser_remaining_controls_e2e.py`; `tests/browser/test_browser_full_app_e2e.py`; `tests/export/test_export.py` | merge-source add/remove, per-item card expansion, default compose collapse/restore, and merge/export payload parity |

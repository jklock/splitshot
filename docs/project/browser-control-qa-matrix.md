...
| PiP | add media, PiP default settings collapse and restore, per-item card toggle/remove, per-item size/opacity/position/sync controls, visible beep-sync analyze/rerun action, first-video secondary sync-analysis status/rerun, shared-lane secondary waveform visibility, GIF PiP media typing | `tests/browser/test_merge_export_contracts.py`; `tests/browser/test_timing_waveform_contracts.py`; `tests/browser/test_browser_remaining_controls_e2e.py`; `tests/browser/test_browser_full_app_e2e.py`; `tests/export/test_export.py` | merge-source add/remove, per-card sync-offset buttons, visible analyze or re-run beep-sync action, per-item card expansion, default PiP collapse/restore, first-analyzed-source sync status/rerun ownership, primary-plus-secondary waveform compare, GIF PiP preview or export parity, and merge/export payload parity |
...
### PiP sync-analysis and waveform compare

- `tests/browser/test_merge_export_contracts.py` should own which merge source is the analyzed PiP target, secondary beep metadata, sync-status rendering, and GIF-vs-video media typing.
- `tests/browser/test_timing_waveform_contracts.py` should own the shared-lane primary-plus-secondary waveform rendering contract and the visible effect of PiP sync offsets on the secondary waveform layer.
- `tests/browser/test_browser_remaining_controls_e2e.py` and `tests/browser/test_browser_full_app_e2e.py` should own the first-video-only analysis rule, rerun behavior, and the manual-only fallback behavior for later PiP videos or GIF media.

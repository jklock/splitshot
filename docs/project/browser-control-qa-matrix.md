...
| Export | output path, preset, quality, show export log modal open/close/backdrop and download, CI Clip1 MP4 proof export | `tests/browser/test_merge_export_contracts.py`; `tests/browser/test_browser_remaining_controls_e2e.py`; `tests/browser/test_browser_full_app_e2e.py`; `tests/export/test_export.py` | export payload capture, export log modal open/close/backdrop and download, merge/export sync truth gate, rendered-output parity, and CI artifact export proof from `docs/Clip1.MP4` |
...
### Media, Compose, and waveform compare

- `tests/browser/test_browser_interactions.py` should own Project-vs-Media control gating and queue edit routing back into Media.
- `tests/browser/test_merge_export_contracts.py` should own which merge source is the analyzed target, secondary beep metadata, and media typing.
- `tests/browser/test_timing_waveform_contracts.py` should own the shared-lane primary-plus-secondary waveform rendering contract.
- `tests/browser/test_browser_remaining_controls_e2e.py` and `tests/browser/test_browser_full_app_e2e.py` should own the first-video-only analysis rule, rerun behavior, and manual-only fallback behavior.
...
- `v1.1` should extend those same suites to cover the shared font vocabulary, marker bubble typography or shape controls, review text-box typography controls, and export parity for those style fields.

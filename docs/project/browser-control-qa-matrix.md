...
| PiP | add media, PiP default settings collapse and restore, per-item card toggle/remove, per-item size/opacity/position/sync controls, first-video secondary sync-analysis status/rerun, shared-lane secondary waveform visibility, GIF PiP media typing | `tests/browser/test_merge_export_contracts.py`; `tests/browser/test_timing_waveform_contracts.py`; `tests/browser/test_browser_remaining_controls_e2e.py`; `tests/browser/test_browser_full_app_e2e.py`; `tests/export/test_export.py` | merge-source add/remove, per-card sync-offset buttons, per-item card expansion, default PiP collapse/restore, first-analyzed-source sync status/rerun ownership, primary-plus-secondary waveform compare, GIF PiP preview or export parity, and merge/export payload parity |

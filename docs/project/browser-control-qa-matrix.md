...
| In / Out | opens the Intro / Outro pane for project-managed boundary video selection through the packaged native picker or browser fallback, independent video/audio fade-in and fade-out durations, WYSIWYG text overlay preview, custom text, selectable match-result fields, and overlay persistence | `tests/domain/test_queue_export_fixes.py`; `tests/browser/test_queue_pane_qa.py`; `tests/export/test_export.py`; `electron/tests/smoke.test.js` | the sidebar remains compact while the pane uses full Intro / Outro terminology; media copies into `IntroOutro/`, both clips keep independent fade and overlay configurations, the selected clip immediately becomes the preview source, match summary fields resolve at combined-render time, and Queue includes only explicitly enabled clips |

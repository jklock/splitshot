...
- [x] `[UI-only]` Re-prove the `v1.0.5` Windows export-font path stays intact
- [x] `[UI-only]` Re-prove packaged/test flows still validate `docs/Clip1.MP4`
- [x] `[UI-only]` Keep browser preview font stacks aligned to the released Windows-safe families

## P0 UI Blockers

- [x] `[UI-only]` Fix PiP playback smoothness first
- [x] `[UI-only]` Replace the flat shell rail with the three-surface model
- [x] `[UI-only]` Keep legacy stage-edit behavior stable while the shell changes
...
- [x] `[Narrow backend support required]` Keep `/api/state` summary-oriented and move heavy reads to dedicated calls

## P1 Shell And Navigation

- [x] `[UI-only]` Add top-level surface switcher
- [x] `[UI-only]` Add persistent context header
- [x] `[UI-only]` Rehome legacy tool panes under the correct surface
- [x] `[UI-only]` Define empty/loading/error/stale/unresolved states

## P1 Single Video

- [x] `[UI-only]` Add output-profile manager
- [x] `[UI-only]` Add profile CRUD UX
- [x] `[UI-only]` Add retained-review source selection
- [x] `[UI-only]` Add Run Window, Metric Captions, Frame Profiles, Lead-In Card, Brand Mark, Subject Track Crop editors
...
- [x] `[UI-only]` Add shared-default editor
- [x] `[UI-only]` Add stage-override editor

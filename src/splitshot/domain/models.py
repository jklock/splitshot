...
def default_merge_source_angle_role(asset: VideoAsset | None = None) -> str:
    # Keep the existing conservative defaults until later controller-side role
    # priority work makes `primary` behavior functional in Compose.
...
def project_from_dict(data: dict[str, Any]) -> Project:
    ...
    secondary_video_is_explicitly_set = "secondary_video" in data
    secondary_video = (
        None
    ...
    merge_pip_enum = _normalize_pip_size(merge_data.get("pip_size", PipSize.MEDIUM.value))
    has_explicit_source_sync = len(merge_sources) == 1 and any(
    ...
    ensure_merge_source_composition_truth(
        project,
        raw_merge_sources=raw_merge_sources,
        secondary_video_is_explicitly_set=secondary_video_is_explicitly_set,
    )
    if len(project.merge_sources) == 1:
        if not has_explicit_source_sync:
            project.merge_sources[0].sync_offset_ms = int(project.analysis.sync_offset_ms)
